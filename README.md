...
- Original Spark analysis notebooks retained with execution outputs and visualisations for reference.
...
│   └── dashboard.png
├── notebooks/
├── plots/                      # Visualisations generated by the analysis notebooks
├── scripts/
│   ├── download_sources.py
...
The notebooks in `notebooks/` document the original Spark-based data preparation, feature engineering, exploratory analysis and demand modelling workflow. Their execution outputs are retained for review, and selected visualisations generated by the notebooks are stored in `plots/`. Source and generated data files are excluded from the repository.
...
The notebooks preserve the complete analytical workflow, while the Databricks implementation provides a reusable medallion pipeline for data processing, quality control and reporting.
...
- [NYC PLUTO](https://www.nyc.gov/site/planning/data-maps/open-data/dwn-pluto-mappluto.page) (optional land-use enrichment)
